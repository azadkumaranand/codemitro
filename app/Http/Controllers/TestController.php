...
class TestController extends Controller
{
    ...
    public function storeTest(Request $request){
        ...
        $filepath = $request->file('thumbnail')->store('thumbnails', 'public_html');
        // return $request;
        // return $filePath;
        Test::create([
            'title'=>$request->title,
    ...
    public function customizeTest(string $id){
        ...
        $result = Test::where('tests.id', $id)
                        ...
                        ->leftjoin('options', 'questions.answer', '=', 'options.id')
                        ->select(
                            ...
                            'tests.thumbnail as test_thumbnail',
    ...
    public function updateTest(Request $request, string $id){
        ...
        if ($test && $test->thumbnail) {
            if (Storage::disk('public_html')->exists($test->thumbnail)) {
                // Delete previous image from the server
                Storage::disk('public_html')->delete($test->thumbnail);
            }
        }
        if($request->hasFile('thumbnail')){
            // return $request->file('thumbnail');
            // $imagePath = $request->file('thumbnail')->store('thumbnails', 'public');
            $imagePath = $request->file('thumbnail')->store('thumbnails', 'public_html');
    ...
    public function showTestResult(){
        ...
        $testResults = TestResult::join('questions', 'test_results.question_id', '=', 'questions.id')
            ...
            ->select('users.name as user_name', 'test_results.user_id as user_id', 'tests.title as test_title',
            ...
            ->groupBy('users.name', 'tests.title', 'test_results.user_id')
            ->get();

        return view('teacher.tests.result.page', compact('testResults'));
    }

    public function deleteTestResultOfUser($userId){

        $result = TestResult::where('user_id', base64_decode($userId))->delete();

...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        return view('frontend.page', ['courses'=>$courses, 'tests'=>$tests, 'crousel'=>$this->courselContent]);
    }
    ...
    public function allTests(){
        $tests = Test::where('status', 'publish')->get();
        return view('frontend.pages.test', ['tests'=>$tests]);
    }
    ...
    public function chapterDetails(string $id){
        ...
        $result = Chapter::where('chapters.id', '=', $decodedId)
        ...
        if(count($result)>0){
            ...
            $nextChapter = Chapter::where('id', '>', $decodedId)
                            ->orderBy('id', 'asc')
                            ->first();
            $next = $nextChapter ? base64_encode($nextChapter->id) : null;

            // Find the previous chapter ID
            $prevChapter = Chapter::where('id', '<', $decodedId)
                            ->orderBy('id', 'desc')
                            ->first();
            $prev = $prevChapter ? base64_encode($prevChapter->id) : null;
            return view('frontend.course.chapterdetails', ['result'=>$result, 'next'=>$next, 'prev'=>$prev]);
        }

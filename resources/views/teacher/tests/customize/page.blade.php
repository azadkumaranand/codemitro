            ...
            <form action="{{ route('teacher.test.update', ['id' => $results[0]->test_id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="my-3">
                            ...
                            <textarea id="description" type="text" rows="5" class="form-control" name="description" id="title">{{ $results[0]->test_desc }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                            ...
                            <label for="thumbnail">
                                <div class="position-relative image-container" style="width: 300px; height: 300px">
                                    ...
                                    <div class="img-box d-flex justify-content-center h-100 w-100 align-items-center">
                                        @if (!empty($results[0]->test_thumbnail))
                                            <img src="{{Storage::url($results[0]->test_thumbnail)}}" alt="" style="width: 100%; height: 100%;">
                                        @else
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option value="draft" @if ($results[0]->test_status == 'draft') selected @endif>Draft</option>
                        <option value="publish" @if ($results[0]->test_status == 'publish') selected @endif>Publish</option>
                    </select>
                </div>
            ...
            <form action="{{ route('teacher.test.update', ['id' => $results[0]->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="my-3">
                            <label for="title" class="form-label">Description</label>
                            <textarea id="description" type="text" rows="5" class="form-control" name="description">{{ $results[0]->description }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="col-lg-6 d-flex justify-content-center flex-column">
                                ...
                                <div class="position-relative image-container" style="width: 300px; height: 300px">
                                    ...
                                    <div class="img-box d-flex justify-content-center h-100 w-100 align-items-center">
                                        @if (!empty($results[0]->thumbnail))
                                            <img src="{{Storage::url($results[0]->thumbnail)}}" alt="thumbnail" style="width: 100%; height: 100%">
                                        @else
                                            <i class="fa fa-camera" style="font-size: 25px;"></i>
                                        @endif
                                    </div>
                ...
                <div class="mb-3">
                    <label for="status" class="form-label">Status</label>
                    <select name="status" id="status" class="form-control">
                        <option value="draft" @if ($results[0]->status == 'draft') selected @endif>Draft</option>
                        <option value="publish" @if ($results[0]->status == 'publish') selected @endif>Publish</option>
                    </select>
                </div>
                <button class="btn btn-dark">Update</button>
            </form>
                        ...
                        @if ($results[0]->test_type == 'mcq')
                            <form action="{{ route('teacher.test.mcq.store') }}" method="POST">
                                ...
                                <div class="mcq-question-container">
                                    <div class="my-3">
                                        <label for="questiondesc" class="form-label">Question</label>
                                        <textarea type="text" rows="5" id="questiondesc" class="form-control" name="question"></textarea>
                                    </div>
